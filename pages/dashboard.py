...
with crop_tab:
    ...
    with crop_col2:
        st.plotly_chart(px.line(crop_data_viz, x='Year', y=selected_crop_feature, 
                                title=f'{selected_crop} {selected_crop_feature} by Year', height=500, 
                                line_shape='spline'))
...
with weather_tab: 
    ...
    weather_data = weather_data.merge(rain_data, left_on='YEAR', right_on='year')
    ...
    with weather_col2:
        st.plotly_chart(px.line(weather_data, 
                                x='YEAR', y=WEATHER_FEATURES[selected_weather_feature], 
                                title=f'{selected_weather_feature} by Year', height=500,
                                line_shape='spline'))

# Crop yield and weather data
with crop_weather_tab:

    X = weather_data.drop(columns=['YEAR', 'year'])
    ...
    with combine_right:
        ...
        fig = go.Figure() 
        fig.add_trace(go.Scatter(x=crop_weather_data['Year'], y=crop_weather_data[crop_feature], 
                                 name=crop_feature, yaxis='y', line_shape='spline'))        
        fig.add_trace(go.Scatter(x=crop_weather_data['Year'], y=crop_weather_data[WEATHER_FEATURES[weather_feature]], 
                                 name=weather_feature, yaxis='y2', line_shape='spline'))
        fig.update_layout(   
            yaxis=dict(title=crop_feature),
            yaxis2=dict(title=weather_feature, overlaying='y', side='right'),
            title_text=f'{crop_feature} vs {weather_feature} by Year',
            legend=dict(
                orientation='h',
